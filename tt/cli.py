...
def main(argv=None):
    ...
    create_parser.add_argument("description", help="Long description", nargs="?")
    create_parser.set_defaults(func=do_create)

    describe_parser = subparsers.add_parser("describe")
    describe_parser.add_argument("name", help="Task name")
    describe_parser.add_argument("description", help="Task description", nargs="?")
    ...
    start_parser.add_argument("time", help="timestamp", default="now", nargs="?")
    start_parser.set_defaults(func=do_start)

    stop_parser = subparsers.add_parser("stop")
    stop_parser.add_argument("time", help="timestamp", default="now", nargs="?")
    stop_parser.set_defaults(func=do_stop)

    edit_parser = subparsers.add_parser("edit")
    edit_parser.add_argument("id", type=int, help="ID of the timer to edit")
    edit_parser.add_argument("--delete", action="store_true", help="delete the timer")
    ...
    report_parser.add_argument(
        "--month", type=int, choices=range(1, 13), help="Month to generate report for"
    )
...
def do_create(args):
    ...
    print('Added task "%s" with description "%s"' % (args.name, args.description))
...
def do_edit(args):
    ...
    if args.start_time or args.stop_time or args.task:
        ...
        if args.start_time:
            ...
            print('Updating start time for timer "%s" to "%s"' % (args.id, start))
        if args.stop_time:
            stop = _parse_timestamp(args.stop_time)
            print('Updating stop time for timer "%s" to "%s"' % (args.id, stop))

        if args.task:
            print('Updating task for timer "%s" to "%s"' % (args.id, args.task))
...
def do_status(args):
    ...
    now = datetime.now(tz_local()).replace(hour=0, minute=0, second=0, microsecond=0)
...
def _parse_timestamp(timestamp_in):
    ...
    """
    timestamp_out = dateparser.parse(timestamp_in, settings=DATEPARSER_SETTINGS)
