...
def datatable():
    table = Datatable(
        table=[{"foo": 1, "bar": 2}], labels=["foobar"], summaries=["bazboom"]
    )
    return table
...
def test_configure_logging(basic_config, verbose):
    ...
    if verbose:
        ...
        basic_config.assert_called_once_with(level=logging.WARNING, format=mock.ANY)


@pytest.mark.parametrize("options", [["create", "foo"], ["create", "foo", "bar"]])
def test_create(options, task_service):
    ...
    if len(options) == 2:
        ...
        task_service.add.assert_called_with(name=options[1], description=options[2])
...
@pytest.mark.parametrize("options", [["tasks"]])
def test_tasks(options, mocker, task_service):
    task_service.list.return_value = iter(
        [("foo", "one"), ("bar", "two"), ("bam", "three")]
    )
    tt.cli.main(options)
    assert task_service.list.called


@pytest.mark.parametrize("options", [["remove", "foo"]])
def test_remove(options, mocker, task_service):
    tt.cli.main(options)
    task_service.remove.assert_called_with(name=options[1])


@pytest.mark.parametrize("options", [["start", "foo", "one day ago"], ["start", "foo"]])
...
def test_start(parse, options, mocker, timer_service):
    ...
    if len(options) == 3:
        parse.assert_called_with(options[2], settings=tt.cli.DATEPARSER_SETTINGS)
    else:
...
def test_stop(parse, options, mocker, timer_service):
    ...
    if len(options) == 2:
        parse.assert_called_with(options[1], settings=tt.cli.DATEPARSER_SETTINGS)
    else:
        parse.assert_called_with("now", settings=tt.cli.DATEPARSER_SETTINGS)
...
def test_from_timerange(mock_datetime, mocker):
    ...
    assert tt.cli.from_timerange(args) == (
        datetime(2018, 2, 23, 0, 0, tzinfo=tz_local()),
        datetime(2018, 2, 24, 0, 0, tzinfo=tz_local()),
    )
...
def test_from_timerange_yesterday(mock_datetime, mocker):
    ...
    assert tt.cli.from_timerange(args) == (
        datetime(2018, 2, 22, 0, 0, tzinfo=tz_local()),
        datetime(2018, 2, 23, 0, 0, tzinfo=tz_local()),
    )
...
def test_from_timerange_week(mock_datetime, mocker):
    ...
    assert tt.cli.from_timerange(args) == (
        datetime(2018, 2, 19, 0, 0, tzinfo=tz_local()),
        datetime(2018, 2, 26, 0, 0, tzinfo=tz_local()),
    )
...
def test_from_timerange_last_week(mock_datetime, mocker):
    ...
    assert tt.cli.from_timerange(args) == (
        datetime(2018, 2, 12, 0, 0, tzinfo=tz_local()),
        datetime(2018, 2, 19, 0, 0, tzinfo=tz_local()),
    )
...
def test_from_timerange_month(mock_datetime, mocker):
    ...
    assert tt.cli.from_timerange(args) == (
        datetime(2018, 2, 1, 0, 0, tzinfo=tz_local()),
        datetime(2018, 3, 1, 0, 0, tzinfo=tz_local()),
    )
...
def test_from_timerange_year(mock_datetime, mocker):
    ...
    assert tt.cli.from_timerange(args) == (
        datetime(2018, 1, 1, 0, 0, tzinfo=tz_local()),
        datetime(2019, 1, 1, 0, 0, tzinfo=tz_local()),
    )
